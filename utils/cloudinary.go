...
// UploadImage uploads an image with optimization
func (s *Cloudinary) UploadImage(file *multipart.FileHeader, folder string) (*UploadResult, error) {
	...
	uploadParams := uploader.UploadParams{
		PublicID:       publicID,
		Folder:         folder,
		Transformation: "f_auto,q_auto,w_1200", // Auto format, quality, max width 1200px
		Overwrite:      &overwrite,
	}
...
// GenerateURL generates optimized URL for different sizes
func (s *Cloudinary) GenerateURL(publicID string, size string) string {
	...
	// Build URL manually with transformation
...
// GetImageVariants returns URLs for different image sizes
